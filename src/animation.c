...
void aq_queue_condense(struct Animation* aq) {
	for (uint8_t i = 0; i < AQ_CAPACITY; ++i) {
		if (!an_is_active(aq[i]) && !an_queue_redraw_background(aq[i])) {
			// empty space found
...
bool aq_submit_animation(struct Animation* aq, struct Card* card, CardStorage* sto, uint24_t x, uint8_t y, uint8_t flags) {
	...
	for (uint8_t i = 0; i < AQ_CAPACITY; ++i) {
		if (!an_is_active(aq[i]) && !an_queue_redraw_background(aq[i])) {
			...
			//an_set_ending_queue_source_redraw(aq[i], true);
			//an_set_queue_redraw_background(aq[i], true);
			return true;
		}
	}
...
void aq_render_and_animate_cards(struct Animation* aq) {
	...
	for (uint8_t i = 0; i < AQ_CAPACITY; ++i) {
		if (an_is_active(aq[i])) {
			...
			animate(&aq[i]);
			if (an_queue_redraw_background_every_frame(aq[i])) {
			...
			if (aq[i].card.life_remaining == 0) {
				if (aq[i].storage != NULL && aq[i].storage->redraw_frames == 0 && an_ending_queue_source_redraw(aq[i])) {
...
void aq_draw_back_sprites(struct Animation* aq) {
	for (uint8_t i = AQ_CAPACITY - 1; i < AQ_CAPACITY; --i) {
		if (an_queue_redraw_background(aq[i])) {
			...
			an_set_queue_redraw_background(aq[i], false);
		}
	}
}